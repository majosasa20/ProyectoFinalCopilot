...
builder.Services.AddControllers()
    .AddJsonOptions(options =>
    {
        ...
        // Permitir indentación para mejor legibilidad
...
// Add CORS - Configuración actualizada para incluir puertos específicos
builder.Services.AddCors(options =>
{
    options.AddPolicy("AllowSpecificOrigins", policy =>
    {
        policy.WithOrigins(
                "https://localhost:7000",     // Puerto WebApp por defecto
                "https://localhost:7001",     // Puerto API por defecto  
                "https://localhost:5001",     // Puerto alternativo
                "https://localhost:5000",     // Puerto HTTP alternativo
                "https://localhost",          // Base localhost
                "https://adventureworks.local"
              )
              .AllowAnyHeader()
              .AllowAnyMethod()
              .AllowCredentials();
...
using (var scope = app.Services.CreateScope())
{
    ...
    {
        logger.LogError("?? API Key no configurada en appsettings.json");
    }
    else if (apiKey.Length < 32)
    {
        logger.LogWarning("?? API Key muy corta. Se recomienda al menos 32 caracteres");
    }
    else
    {
    ...
    logger.LogInformation("?? CORS configurado para múltiples orígenes localhost");
    logger.LogInformation("?? API ejecutándose en: {Urls}", string.Join(", ", app.Urls));
}
